...
def show_tls(request):
    ...
    return render(request, "h/show_tls.html", {'subject_list': contacts,
...
def show_heartbeat(request):
    http_seq = HeartbeatRes.objects.filter(is_http=True)
    https_seq = HeartbeatRes.objects.filter(is_http=False)
    return render(request, "result_show/show_heartbeat.html", {"http_seq": http_seq, 
                                                               "https_seq": https_seq,
                                                               "num_http": len(http_seq),
                                                               "num_https": len(https_seq)})