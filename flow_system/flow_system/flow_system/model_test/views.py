...
from model_test.heartbeat.heartbeat import heartbeat_detection
...
使用筛选出的有效的7维特征进行训练
'''
def test_tls_7(request):
    # TODO 使用已有样本进行训练
    # TODO 对采集的新样本进行检测
    return render(request, 'model_testtest_tls_7.html')

'''
使用心跳方法对样本进行检测
'''
def test_heartbeat(request):
    # 使用心跳方法对新样本进行检测
    heartbeat_detection()
    return render(request, 'model_test/test_heartbeat.html')